...
const getAnalogousColors = (hex: string) => {
    ...
    return [
        rgbToHex(rotate(r), g, b),
        rgbToHex(rotate(r), rotate(g), b),
    ];
};
...
const PaletteGenerator = () => {
    ...
    const [complementaryColor, setComplementaryColor] = useState<string>('#83c512');
    const [analogousColors, setAnalogousColors] = useState<string[]>(['#b83aed', '#b876ed']);
    const [triadicColors, setTriadicColors] = useState<string[]>(['#ed7c3a', '#3aed7c']);
            ...
            <div className='flex sm:flex-row flex-col gap-2 sm:gap-0'>
                <div>
                    <div className="flex flex-col items-center ">
                        <div className="w-32 h-16" style={{ backgroundColor: complementaryColor }}></div>
                    ...
                    <div className="flex">
                        {analogousColors.map((color, index) => (
                            <div key={index} className="flex flex-col items-center ">
                                <div className="w-32 h-16" style={{ backgroundColor: color }}></div>
                                <span>{color}</span>
                            </div>
                        ))}
                    </div>
