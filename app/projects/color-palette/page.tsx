    ...
    <main className="h-screen w-full flex flex-row overflow-hidden">
      {colors.map((color, index) => (
        ...
        >
          ...
          <div className={cn("font-mono text-lg font-bold mb-4", getTextColor(color))}>{color.toUpperCase()}</div>
          ...
          >
            ...
            >
              {lockedColors[index] ? <Lock className="h-6 w-6" /> : <Unlock className="h-6 w-6" />}
            </Button>
          </div>
          ...
          >
            <p className="text-sm">
              Press <kbd className="px-2 py-1 bg-black/10 rounded">spacebar</kbd> to generate new colors
            </p>
          </div>
